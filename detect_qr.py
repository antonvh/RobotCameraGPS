...
while True:
    ...
    for m in mu:
        if m['m00'] != 0:
            # Calculate a list of centroids, to check if the found squares are centered over each other.
            # ...or later calculate distances
    ...
    if mark > 2:
        ...
        if (top < len(contours) and right < len(contours) and bottom < len(contours) and cv2.contourArea(
            ...
            #Convert cv2 image to PIL image for zbar
